...
-(id)showDropDownFromButton:(UIButton *)button WithHeight:(CGFloat *)height WithContentArray:(NSArray *)arrayContent WithImageArray:(NSArray *)arrayImages WithDirection:(NSString *)direction {
    ...
    if (self) {
        ...
        self.list = [NSArray arrayWithArray:arrayContent];
        self.imageList = [NSArray arrayWithArray:arrayImages];
        ...
        table.backgroundColor = [UIColor colorWithRed:85.0f/255.0f green:45.0f/255.0f blue:0.0f/255.0f alpha:1];
...
//    [btnSender addSubview:imgView];
    indexStation = indexPath.row;

    [self myDelegate];
    
  //    [(HomeVC * )delegategetstation setindexvalue:indexStation];
}

- (void) myDelegate
{
    [self.delegate niDropDownDelegateMethod:self withindex:indexStation];
}
